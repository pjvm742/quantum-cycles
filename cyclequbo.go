...
func constructqubo(g cwdgraph, penmult float64, vdis bool, adis bool) (qubo, transltable) {
	a := g.arcc
	n := len(a)
	...
	for i := 0; i < n; i++ {
		...
		addquadpen(QUBO, indices, values, penmult)
	}
	for j := 0; j < n; j++ {
		var indices []int
		var values []int
...
func main() {
	...
	if !absmult {
		...
		fmt.Fprintln(os.Stdout, avg)
		penmult *= avg
	}
	qubomatrix, tlt := constructqubo(reducedg, penmult, vdis, adis)
...
func writeQUBO(filename string, problem qubo) {
	...
	for i := 0; i < n; i++ {
		row := make([]string, n)
		for j := 0; j < n; j++ {
			row[j] = fmt.Sprintf("%.4g", problem[i][j])
		}
		rep[i] = row
	}
	w.WriteAll(rep)
